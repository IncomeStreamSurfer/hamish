...
def extract_sitemap_data(xml_file_path, search_terms, num_urls=20):
    ...
    for url in tqdm(root.findall('ns:url', namespaces), desc="Processing URLs"):
        ...
        if image is not None:
            ...
            # Check if any search term is in loc or image_title
            if any(term.lower() in loc.lower() for term in search_terms) or \
               any(term.lower() in image_title for term in search_terms):
